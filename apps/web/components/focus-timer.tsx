...
export function FocusTimer() {
  ...
  // Ambient sound state
  const [ambientSource, setAmbientSource] =
  ...
  const dailyProgress = Math.min(
    (todayStats.completedSessions / settings.dailyGoal) * 100,
    100
  );
            ...
            setStats((prev) => {
                ...
                return prev.map((s) =>
                  s.date === today
                    ? {
                        ...
                        totalFocusMinutes:
                          s.totalFocusMinutes +
                          Math.round(settings.workDuration / 60),
                      }
                    : s
                );
              }
              return [
                ...prev,
                {
                  date: today,
                  completedSessions: 1,
                  totalFocusMinutes: Math.round(settings.workDuration / 60),
                },
              ];
            });
              ...
              <div className="flex items-end gap-1 h-16">
                {Array.from({ length: 7 }).map((_, i) => {
                  ...
                  const maxSessions = Math.max(
                    ...stats.map((s) => s.completedSessions),
                    1
                  );
              ...
              <div className="flex items-center justify-between gap-3">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setSettings((s) => ({
                      ...s,
                      ambientSound: e.target.value as AmbientSound,
                    }))
                  }
